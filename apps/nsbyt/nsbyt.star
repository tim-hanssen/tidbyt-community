...
def main(config):
    ...
    if len(stops) == 1:
        return render.Root(child = renderTrain(stops[0], skiptime))

    return render.Root(
        show_full_animation = True,
        child = render.Column(
            children = [
                renderTrain(stops[0], skiptime),
                render.Box(
                    color = "#ffffff",
                    width = 64,
                    height = 1,
                ),
                renderTrain(stops[1], skiptime),
            ],
        ),
    )

def renderTrain(stop_info, skiptime):
    ...
    departureTimeText = humanize.relative_time(time.parse_duration("%im" % skiptime) + time.now(), parse_time(stop_info["actualDateTime"]))
    ...
    if stop_info["cancelled"] == True:
        ...
        if stop_info.get("primaryMessage"):
            backgroundColor = CANCELED_BACKGROUND_COLOR
            message = stop_info["primaryMessage"]
                ...
                stops.append(
                    {
                        ...
                        "primaryMessage": trip["primaryMessage"]["message"]["text"],
                    },
                )
        else:
            stops.append(
...
def get_schema():
    return schema.Schema(
        ...
        fields = [
            ...
            schema.Toggle(
                id = "show_including_skiptime",
                name = "Show times inlcuding offset",
                desc = "Shows the time you should leave from home instead of the train run time.",
                icon = "clock",
                default = False,
            ),
        ],
